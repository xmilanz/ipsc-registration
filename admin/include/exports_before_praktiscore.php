...
// vybereme zavodniky, kteri zaplatitli nebo jsou v prematchi nebo rozhodci nebo nemusí platit
    $query="select * from $table where Squad>=0 AND (Zaplaceno='on' OR RO='on' OR POM='on' OR VIP='on') ORDER BY Cislo;";
...
";
    ...
    $csvDataPraktiscore.=",$z[Jmeno],$z[Prijmeni],,$z[Alias],$z[Mail],$z[Squad],,,$z[Pidiv],$z[Pifak],,,,,$z[Cislo],$z[Region],,"."\r\n";
...
//  fclose($fh);
  ...
  $fh = fopen('./xml/praktiscore_shooters.csv', 'w');
  $xmlData="member,firstname,lastname,name,alias,email,squad,age,gender,division,powerfactor,class,special,checkin,team,shooter,region,state,rocode"."\r\n";
  $xmlData.=iconv("UTF-8", "Windows-1250", $csvDataPraktiscore);
  fwrite($fh, $xmlData);
  fclose($fh);
...
<rs:data>
;;;	<z:row MatchId=\'1\' MatchName=\'Eggenberg CUP 202X - X. kolo\' MatchDt=\'1970-01-01T09:00:00\' ClubId=\'1\' Chrono=\'False\'
